...
public class P3Wrapper {
	...
	public static class BinarySearchTree<K, V> implements Map<K, V> {
		...
		/*
		 * Implement a member method inFullPath which determines if a key is 
		 * located on a BST where each of its ancestors have two children 
		 * (i.e., left and child children). Do not count e as its own ancestor. The method
		 * receives as parameter the key of the value to be searched in the BST. The method
		 * return true is e lies on such path, or false otherwise. The method returns
		 *  false if the element e is not found on the  BST, or if e is the root. 
		 */
		...
		public boolean inFullPathAux(K key, BinaryTreeNode<MapEntry<K, V>> N) {
			return false;
